...
<div class="container pt-5 pb-3">
    <div class="search px-2 pt-2 pb-5">
        {{-- <form class="px-5" action="/search_voitures" methode="get"> --}}
                ...
                <div class="col-md-3">
                    <select id="" class="card fw-semibold mb-3 py-1 px-3 mb-md-0 px-md-3 w-100" name="marque">
                        ...
                        @foreach($marques as $m)
                            <option value="{{$m}}" {{ $m === old('marque') ? 'selected' : '' }} > {{$m}} </option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    <select id="" class="card fs-5 mb-3 py-1 px-3 mb-md-0 py-md-0 px-md-3 w-100" name="transmission">
                        <option value="" class="text-muted">transmission</option>
                        @foreach($transmission as $t)
                            <option value="{{$t}}" {{ $t === old('transmission') ? 'selected' : '' }}> {{$t}} </option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    <select id="" class="card fs-5 mb-3 py-1 px-3 mb-md-0 py-md-0 px-md-3 w-100" name="carburant">
                        <option value="" class="text-muted">types de carburants</option>
                        @foreach($carburant as $c)
                            <option value="{{$c}}" {{ $c === old('carburant') ? 'selected' : '' }}> {{$c}} </option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-3">
            ...
            {{-- <div class="row py-3 py-md-2 p-lg-3 bg-white rounded border shadow justify-content-center">
                <div class="col-5">
                    <select id="" class="card fw-semibold mb-3 py-1 px-3 mb-md-0 px-md-3 w-100" name="marque">
                        <option value="" class="fw-semibold text-muted">Marques</option>
                        @foreach($marques as $m)
            ...
            <div class="col-lg-3 mb-4 pe-lg-3 px-0">
                <div class="box-filter card shadow">
                    <div class="filter">
                        ...
                        {{-- <div class="px-2 py-2"> --}}
                            {{-- <form oninput="result.value = slider.value" action="" methode="get" class="px-1">
                                <h5 class="text-center text-lg-start py-1 fw-semibold">Select Price</h5>
                                <input type="range" name="prix" class="py-1 w-100" id="slider" min="{{$prix_min}}" max="{{$prix_max}}" step="100" value="{{$prix_min}}"> <br />
                                <h5 class="text-center text-lg-start fw-semibold">The Price is <output name="result" for="slider">{{$prix_min}}</output> DH</h5>
                            </form> --}}

                            {{-- <div class="item_filter_group" style="background-size: 100%; background-repeat: no-repeat;">
                                <h4>Price ($)</h4>
                                  <div class="price-input" style="background-size: 100%; background-repeat: no-repeat;">
                                    <div class="field" style="background-size: 100%; background-repeat: no-repeat;">
                                      <span>Min</span>
                                  ...
                                  <div class="slider" style="background-size: 100%; background-repeat: no-repeat;">
                                    <div class="progress" style="background-size: 100%; background-repeat: no-repeat; left: 15.1%; right: 47.9%;"></div>
                                  </div>
                                  <div class="range-input" style="background-size: 100%; background-repeat: no-repeat;">
                            ...
                            <div class="row pt-4 pb-2 px-3">
                                <div class="col-12 mb-4">
                                    <select id="" class="card select fw-semibold text-capitalize mb-3 py-1 px-3 mb-md-0 px-md-3 w-100" name="prix">
                                        ...
                                        @foreach($prix as $p)
                                            <option value="{{$p}}" {{ $p === old('prix') ? 'selected' : '' }} > {{$p}} </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-12 mb-4">
                                ...
                                <div class="col-6 col-lg-12 text-lg-center mb-4">
                                    <button type="reset" class="btn bg-primary btn_reset text-light fs-5 w-100 py-0">Reset Filter</button>    
                                </div>
                            </div>
                        </form>                
                    </div>
